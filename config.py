...
def set_config(args):
    if "hiv" in args.dataset:
        ...
        # Add configuration for IP Geolocation dataset
        args.criterion = 'geo_distance'  # Custom criterion for geographic distance
        args.task = 'geo_regression'  # Task is regression for lat/lon
        args.node_method = 'add'  # Node features method
        args.edge_method = 'embedding'  # Edge features method
        args.mpnn = 'GCN'  # Use GCN as the MPNN
        args.pool = 'None'  # No pooling for node-level tasks
        args.projection = 'mlp'  # Use MLP for final projection
    else:
        args.criterion = 'accuracy'
        args.task = 'multi_class'
...
def gene_arg():
    ...
    # Add arguments for IP geolocation
    group.add_argument('--loss1_weight', type=float, default=0.001,
                       help='Weight for the latitude loss')
    group.add_argument('--delay_weight', type=float, default=0.001,
                       help='Weight for the delay loss')
    ...
    group.add_argument('--runs', type=int, default=10)  #修改int为str
